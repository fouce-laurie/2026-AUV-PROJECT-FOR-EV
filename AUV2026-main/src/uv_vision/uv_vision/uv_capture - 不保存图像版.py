...
DATAS_DIR = WORKSPACE_ROOT / 'datas'
        ...
        if s:
            ...
            if self.opt.down_params[0] != "none":
                ...
                if cnt_front == 2:
                    ...
                    filename = os.path.join(str(WORKSPACE_ROOT / 'img' / 'front'), f"{timestamp}.jpg")
        ...
        if s:
            ...
            if self.opt.down_params[0] != "none":
                ...
                if cnt_down == 2:
                    now = datetime.now()
                    timestamp = now.strftime("%Y%m%d_%H%M%S%f")
                    filename = os.path.join(str(WORKSPACE_ROOT / 'img' / 'down'), f"{timestamp}.jpg")
                    cv2.imwrite(filename, full)
                    cnt_down = 0"""
