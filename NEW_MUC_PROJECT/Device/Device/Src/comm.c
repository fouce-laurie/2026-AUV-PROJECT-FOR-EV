...
// Added variables according to uv_hmu.py callbacks
uint8_t servo_num = 0;
float servo_angle = 0.0f;
uint8_t magnet_state = 0;
float target_speed[2] = {0.0f, 0.0f};
...
void CommInit(void)
{
	...
	HAL_UART_Receive_IT(&huart2, uart2rec.buf, 1);
...
void ImuSolve(uint8_t *buf)
{
    ...
    if (buf[0] == 0xfa && buf[1] == 0xaf && check == buf[91]) // 校验帧头
    {
        ...
        memcpy(&(imu_data[0]), buf + 99, 8); // x y
        memcpy(&(imu_data[2]), buf + 46, 4); // Z。
        imu.imustate = buf[129];              // 导航状态
        imu.dvlstate = buf[115]&1;
...
void CmdSolve(uint8_t *buf)
{
    switch (buf[2])
    {
        ...
        case 0x04: // IMU状态设置
            dvlstate = buf[3];
						refresh=1;
//                if (buf[3] == 1) { dvl_startup(&huart5); }
//                if (buf[3] == 0) { dvl_shutdown(&huart5); }
        ...
        case 0x07: // 推力曲线
            ...
            memcpy(&(thrustcurve[buf[3]].thrust[3]), buf + 32, 4);	
					static uint8_t data[100];
						...
						memcpy(data+5, buf + 4, 32);
						data[37]=0xFB;
						data[38]=0xBF;
						HAL_UART_Transmit_DMA(&huart6,data,39);
						
						
            break;
        
        case 0x08: // LED控制 / 目标速度设置 (重用0x08)
            if (buf[5] == 0xfb && buf[6] == 0xbf) // LED控制(payload: BB)
            {
                led[0] = (buf[3] == 0) ? 0 : 1;
                led[1] = (buf[4] == 0) ? 0 : 1;
            }
            else if (buf[11] == 0xfb && buf[12] == 0xbf) // 目标速度下行(payload: ff)
            {
                memcpy(&target_speed[0], buf + 3, 4);
                memcpy(&target_speed[1], buf + 7, 4);
            }
            break;
        ...
        case 0x10: // 推力矩阵数据转发至下位机
            {
                static uint8_t data_10[26];
                data_10[0] = 0xFA;
                data_10[1] = 0xAF;
                data_10[2] = buf[2]; // 0x10
                data_10[3] = buf[3]; // rw_flag (0: 读, 1: 写)
                
                // 拷贝A_1, A_2, B, C, _2b 共 20 字节 (5个float)
                memcpy(data_10 + 4, buf + 4, 20);
                data_10[24] = 0xFB;
                data_10[25] = 0xBF;
                HAL_UART_Transmit_DMA(&huart6, data_10, 26);
            }
            break;
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart) // 串口接收中断
{
    ...
    if (huart == &huart2)
    {
        ...
        if (uart2rec.buf[uart2rec.cnt - 1] == 0xfa && uart2rec.buf[uart2rec.cnt] == 0xaf && uart2rec.cnt > 0)
        {
            uart2rec.cnt    = 1;
            uart2rec.buf[0] = 0xfa;
            uart2rec.buf[1] = 0xaf;
        }
        if (uart2rec.buf[uart2rec.cnt - 1] == 0xfb && uart2rec.buf[uart2rec.cnt] == 0xbf && uart2rec.cnt > 0)
        {
            CmdSolve(uart2rec.buf);
            uart2rec.cnt = 201; // 使缓冲计数归零
        }
        if (uart2rec.cnt > 200) // 防止缓冲溢出
            uart2rec.cnt = 0;
        else
            uart2rec.cnt++;
